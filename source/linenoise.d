...
private int getCursorPosition(int ifd, int ofd) {
    ...
    if (buf[0] != KEY_ACTION.ESC || buf[1] != '[')
        return -1;
    if (sscanf(buf_ptr + 2, "%d;%d", &rows, &cols) != 2)
        return -1;
    return cols;
}
    ...
    if (lc[].length == 0) {
        ...
        while (!stop) {
            ...
            switch (c) {
                ...
                if (i < lc[].length) {
                    nwritten = snprintf((*ls).buf, (*ls).buflen, "%s", cast(char*)lc[][i].toStringz);
...
private int linenoiseEdit(int stdin_fd, int stdout_fd, char* buf, size_t buflen, const char* prompt) {
    ...
    linenoiseState l = {
        ifd: stdin_fd, ofd: stdout_fd, buf: buf, buflen: buflen, prompt: prompt, plen: strlen(prompt), oldpos: 0, pos: 0, len: 0,
        cols: getColumns(stdin_fd, stdout_fd), maxrows: 0, history_index: 0
    };
    ...
    while (1) {
        ...
        switch (c) {
            ...
            if (read(l.ifd, seq_ptr, 1) == -1)
                break;
            if (read(l.ifd, seq_ptr + 1, 1) == -1)
                break;
...
extern (C) char* linenoise(const char* prompt) {
    ...
    if (!isatty(STDIN_FILENO)) {
        ...
        printf("%s", prompt);
        fflush(stdout);
        if (fgets(buf_ptr, LINENOISE_MAX_LINE, stdin) == null)
            return null;
        len = strlen(buf_ptr);
        while (len && (buf[len - 1] == '\n' || buf[len - 1] == '\r')) {
            len--;
...
private extern (C) void linenoiseAtExit() {
    disableRawMode(STDIN_FILENO);
    freeHistory();
}
...
extern (C) int linenoiseHistoryLoad(const char* filename) {
    ...
    while (fgets(buf_ptr, LINENOISE_MAX_LINE, fp) != null) {
        char* p;
